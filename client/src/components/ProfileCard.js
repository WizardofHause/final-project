...
export default function ProfileCard({ user_profile }) {
    ...
    return (
        <div class='bg-stone-100 border-4 border-stone-300 rounded-lg p-6 text-stone-600 font-semibold ml-44'>
            { birthday ? <p class='border-2 border-red-300 bg-red-100 py-1 px-2 m-2 text-red-600 font-black uppercase text-center'>HAPPY BIRTHDAY!</p> : null}
            ...
            { birthday ? <p class='border-2 border-red-300 bg-red-100 py-1 px-2 m-2 text-red-600 font-black uppercase'>WE HOPE IT'S A DAY TO REMEMBER</p> : null}
        </div>
    )
}