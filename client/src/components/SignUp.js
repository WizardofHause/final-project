...
function SignUp({ updateUser, fetchMemories }) {
            ...
            .then(res => {
                if (res.ok) {
                    res.json().then(user => {
                        updateUser(user)
                        fetchMemories()
                        history.push(`/user_bank`) // <-- NEW USERS ROUTED IMMEDIATELY TO USER PROFILE PAGE
                    })
                } else {
    ...
    // CHANGE HANDLER FOR NEW USER FORM INPUTS
    ...
    return (
        <div class='flex flex-col justify-center items-center h-screen'>
            ...
            <form onSubmit={onSubmit} class='text-center mx-auto'>
                ...
                <input
                    class='border-4 border-stone-700 hover:border-indigo-900 rounded h-8 text-indigo-900 font-semibold bg-transparent focus:bg-indigo-50'
                    type='text'
                    name='name'
                    value={name}
