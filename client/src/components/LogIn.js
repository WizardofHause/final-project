            ...
            .then(res => {
                if (res.ok) {
                    res.json().then(user => {
                        updateUser(user)
                        fetchMemories()
                        history.push('/bank') // <-- RETURNING USERS ROUTE TO MAIN TIMELINE FEED
                    })
                } else {
        ...
        <div class='flex flex-col justify-center items-center h-screen m-3'>
            ...
            <form onSubmit={onSubmit}>
                <div class='text-center mx-auto'>
                    ...
                    <input
                        class='border-4 border-stone-700 hover:border-teal-700 rounded h-8 text-teal-700 font-semibold bg-transparent focus:bg-teal-50'
                        type='text'
                        name='name'
                        value={name}
            ...
            {/* HANDLE ERROR MESSAGE DISPLAY FOR LOGIN VALIDATIONS */}
            {errors.length > 1 ? <div class='border-2 border-red-300 bg-red-100 py-1 px-2 m-2 text-red-600 font-black uppercase'>{errors}</div> : null}
        </div>
    )
}
